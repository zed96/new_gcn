...
    必须使用python pickle模块保存上面的所有对象。
    ...
    for i in range(len(names)):
        with open("{}/ind.{}.{}".format(data_str, data_str, names[i]), 'rb') as f:
            if sys.version_info > (3, 0):
                objects.append(pkl.load(f, encoding='latin1'))
            else:
                objects.append(pkl.load(f))
    x, y, tx, ty, allx, ally, graph = tuple(objects)
    test_idx_reorder = parse_index_file("{}/ind.{}.test.index".format(data_str,data_str))
    test_idx_range = np.sort(test_idx_reorder)

    # 构建标准化特征矩阵 (2708,1433)
    if data_str == 'citeseer':
...
'''
#preprocessing for link prediction

'''

def load_data_for_link_prediction(data_str):
